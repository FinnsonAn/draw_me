...
<div class="page-heading"> <h2>会员设置</h2> </div>
    ...
    <form action="" method="post" class="form-horizontal form-validate" enctype="multipart/form-data" >
                ...
                <div class="form-group">
                    <label class="col-sm-2 control-label">绑定旧会员数据</label>
                    <div class="col-sm-9 col-xs-12">
                    <?php if(cv('sysset.member.edit')) { ?>
                        <label class="radio radio-inline">
                              <input type="radio" name="data[bind_old_member]" value="0" <?php  if(empty($data['bind_old_member'])) { ?>checked<?php  } ?>/> 关闭
                        </label>
                       <label class="radio radio-inline">
                              <input type="radio" name="data[bind_old_member]" value="1" <?php  if($data['bind_old_member']==1) { ?>checked<?php  } ?>/> 开启
                        </label>
                        <?php  } else { ?>
                        <input type="hidden" name="data[bind_old_member]" value="<?php  echo $data['leveltype'];?>" />
