...
abstract class WeModuleWxapp extends WeBase {
	...
	public function __call($do, $arguments) {
		$dir = IA_ROOT . '/addons/' . $this->modulename . '/core/wxapp';
		$file = "$dir/{$arguments[version]}/{$do}/{$arguments[page]}.php";
		if (!file_exists($file)) {
			...
			if (!empty($version_path_tree)) {
				foreach ($version_path_tree as $path) {
					$file = "$path/{$do}/{$arguments[page]}.php";
					if (file_exists($file)) {
						break;
					}
				}
