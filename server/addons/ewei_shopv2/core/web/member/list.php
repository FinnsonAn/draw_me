...
class List_EweiShopV2Page extends WebPage 
{
	...
	public function add()
	{
		...
		if ($_W['ispost']) 
		{
			...
			if ( empty($mobile))
			{
				show_json(0, array('message' => '手机号不能为空!'));
			}
			$level = intval($_GPC['level']);
			$group = intval($_GPC['groupid']);
			$is_exsit = m('member')->getMobileMember($mobile);
			if ($is_exsit)
			{
				show_json(0, array('message' => '手机号已被占用!'));
			}
			$openid = 'sys_' . md5(time() . random(4));
			$data = array(
				'uniacid'  => $_W['uniacid'],
				'openid'   => $openid,
				'mobile'   => $mobile,
			...
			$id = pdo_insertid();
			// 绑定旧会员数据
			if ($memSet['bind_old_member'])
			{
				$old_member = pdo_fetch('select * from '. tablename('ewei_shop_member_import') .
							' where mobile = :mobile and uniacid = :uniacid and enabled = 0', 
							array(':mobile' => $mobile, 'uniacid' => $_W['uniacid']));
				if ($old_member)
				{
					$update = array(
						'credit2'  => $old_member['money'],
						'old_credit2'  => $old_member['money'],
						'credit1'  => $old_member['integral'],
						'level'    => $old_member['level'],
						'realname' => $old_member['realname'],
						'birthday' => $old_member['birthday'],
						'birthtype' => $old_member['birthtype'],
						'isagent'  => $old_member['isagent'],
						'status'   => $old_member['isagent']
					);
					pdo_update('ewei_shop_member', $update, array('id' => $id));
					pdo_update('ewei_shop_member_import', array('enabled' => 1), array('id' => $old_member['id']));
				}
			}
			plog('member.list.add', '添加会员 ID: ' . $id);
			show_json(1, array('url' => webUrl('member/list')));
		}
		
		include $this->template('member/create');
	...
	public function detail() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (empty($member['mobile']) && !empty($data['mobile']))
			{
				// 绑定旧会员数据
				if ($memSet['bind_old_member'])
				{
					$old_member = pdo_fetch('select * from '. tablename('ewei_shop_member_import') .
								' where mobile = :mobile and uniacid = :uniacid and enabled = 0', 
								array(':mobile' => $data['mobile'], 'uniacid' => $_W['uniacid']));
					if ($old_member)
					{
						$data['credit2'] = $member['credit2'] + $old_member['money'];
						$data['credit1'] = $member['credit1'] + $old_member['integral'];
						if ($old_member['level'] > 0 && empty($data['level']))
						{
							$data['level'] = $old_member['level'];
						}
						if ($old_member['realname'] && empty($member['realname']))
						{
							$data['realname'] = $old_member['realname'];
						}
						if ($old_member['isagent'] && !$member['isagent'])
						{
							$data['isagent'] = $old_member['isagent'];
							$data['status']  = $old_member['isagent'];
						}
						if ($old_member['birthday'] && empty($member['birthday']))
						{
							$data['birthday'] = $member['birthday'];
						}
						$data['birthtype'] = $member['birthtype'];
						$data['old_credit2'] = $old_member['money'];
						pdo_update('ewei_shop_member_import', array('enabled' => 1), array('id' => $old_member['id']));
					}
				}
			}
	...
	public function find()
	{
		...
		$sql = 'select id, avatar, nickname, realname, mobile, openid, credit1, credit2 from '. tablename('ewei_shop_member') .
				' where ' . $where . ' and uniacid = ' . $_W['uniacid'];
		$data = pdo_fetch($sql);
		if (!$data)
		{
			show_json(0, array('msg' => '没有找到会员数据'));
		}
		else
		{
			$data['credit2'] = priceFormat1($data['credit2'], 2);
			show_json(1, $data);
		}
	}
}
