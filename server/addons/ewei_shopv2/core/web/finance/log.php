...
class Log_EweiShopV2Page extends WebPage 
{
	...
	public function printer()
	{
		...
		/* 搜索条件 */
		$con = ' uniacid = '. $_W['uniacid'] .' and status = 1 and type = 0';
		$con1 = $con . ' and rechargetype = \'system\' and money > 0';  // 充值
		$con2 = $con . ' and rechargetype = \'system\' and money < 0';  // 储值扣款
		$con3 = $con . ' and rechargetype = \'wechat\' and money > 0';  // 线上微信充值
		$con4 = ' uniacid = '. $_W['uniacid'] .' and status = 1 and money > 0'; // 线下消费
		$con5 = ' uniacid = '. $_W['uniacid'] .' and status >= 1 and realprice > 0';  // 线上微信支付
		$con6 = ' uniacid = '. $_W['uniacid'] .' and status=3 and realmoney > 0';  // 线上微信支付
		...
		if (!(empty($starttime)) && !(empty($endtime))) 
		{
			...
			$con1 .= ' and createtime >= '. $starttime .' and createtime<'. $endtime .' ';
			$con2 .= ' and createtime >= '. $starttime .' and createtime<'. $endtime .' ';
			$con3 .= ' and createtime >= '. $starttime .' and createtime<'. $endtime .' ';
			$con4 .= ' and createtime >= '. $starttime .' and createtime<'. $endtime .' ';
			$con5 .= ' and paytime >= '. $starttime .' and paytime<'. $endtime .' ';
			$con6 .= ' and paytime >= '. $starttime .' and paytime<'. $endtime .' ';
		}
		
		
		// 充值统计
		$data['recharge'] = pdo_fetchcolumn('select ifnull(sum(money), 0) from '. tablename('ewei_shop_member_log') .
				' where ' . $con1);
		$data['recharge'] = priceFormat1($data['recharge'], 2);
		//储值扣款统计 
		$data['credit2_pay'] = abs(pdo_fetchcolumn('select ifnull(sum(money), 0) from '. tablename('ewei_shop_member_log') .
				' where ' . $con2));
		$data['credit2_pay'] = priceFormat1($data['credit2_pay'], 2);		
		// 线上充值统计
		$data['recharge_xcx'] = pdo_fetchcolumn('select ifnull(sum(money), 0) from '. tablename('ewei_shop_member_log') .
				' where ' . $con3);	
		$data['recharge_xcx'] = priceFormat1($data['recharge_xcx'], 2);		
		// 线下消费统计
		$data['offline_pay'] = pdo_fetchcolumn('select ifnull(sum(money), 0) from '. tablename('ewei_shop_member_offline') .
				' where ' . $con4);		
		$data['offline_pay'] = priceFormat1($data['offline_pay'], 2);		
		// 订单
		$data['order'] = pdo_fetchcolumn('select ifnull(sum(realprice), 0) from '. tablename('ewei_shop_order') .
				' where ' . $con5);		
		$data['order'] = priceFormat1($data['order'], 2);
		// 返现
		$data['commission'] = pdo_fetchcolumn('select ifnull(sum(realmoney), 0) from '. tablename('ewei_shop_commission_apply') .
				' where ' . $con6);		
		$data['commission'] = priceFormat1($data['commission'], 2);
		
		// 充值子类统计
		$data['recharges'] = array('cash' => 0, 'wechat' => 0, 'bankcard' => 0, 'alipay' => 0);
		$logs = pdo_fetchall('select money, payment from '. tablename('ewei_shop_member_log') .' where ' . $con1);
		foreach($logs as $k => $row)
		{
			if (!empty($row['payment']))
			{
				$payment = $row['payment'];
				$data['recharges'][$payment] += $row['money'];
			}
		}
		
		// 线下消费子类统计
		$data['offlines'] = array('cash' => 0, 'wechat' => 0, 'bankcard' => 0, 'alipay' => 0);
		$logs = pdo_fetchall('select money, payment from '. tablename('ewei_shop_member_offline') .' where ' . $con4);
		foreach($logs as $k => $row)
		{
			if (!empty($row['payment']))
			{
				$payment = $row['payment'];
				$data['offlines'][$payment] += $row['money'];
			}
		}
