	...
	protected function orderData($status, $st)
	{
		...
		if (!(empty($_GPC['searchfield'])) && !(empty($_GPC['keyword']))) {
			...
			if ($searchfield == 'ordersn') {
				...
				$condition .= ' AND o.author = :keyword';
			}
		...
		if (!(empty($list))) {
			...
			foreach ($list as &$value ) {
				...
				$author = p('case')->getAuthor($value['author'], 'id, name');
				$value['author'] = "(id:".$author['id'] .")" . $author['name'];
		...
		if ($_GPC['export'] == 1) {
			...
			$columns = array(
				...
				array('title' => '画师', 'field' => 'author', 'width' => 12),
