...
<div class="page-heading"> 
    <span class='pull-right'>
        <?php if(cv('shop.nav.add')) { ?>
        	<a class='btn btn-primary btn-sm' href="<?php  echo webUrl('shop/nav/add', array('group_id' => $group_id))?>"><i class='fa fa-plus'></i> 添加导航</a>
        <?php  } ?>
    </span>
    <h2>导航管理</h2> </div>
...
<form action="./index.php" method="get" class="form-horizontal form-search" role="form">
    ...
    <div class="page-toolbar row m-b-sm m-t-sm">
        ...
        <div class="col-sm-6 pull-right">
            ...
            <select name='group_id' class='form-control input-sm select-sm' style="width:150px;">
               <option value="">
                 位置
               </option>
               <?php  if(is_array($groups)) { foreach($groups as $row) { ?>
               <option value="<?php  echo $row['gid'];?>" <?php  if($_GPC['group_id']==$row['gid']) { ?>selected="selected"<?php  } ?>><?php  echo $row['title'];?></option>
               <?php  } } ?>
            </select>
                ...
                <td>
                    ...
                    <td style="text-align:left;">
                        <?php if(cv('shop.nav.view|shop.nav.edit')) { ?>
                        	<a href="<?php  echo webUrl('shop/nav/edit', array('id' => $row['id'], 'group_id' => $group_id))?>" class="btn btn-default btn-sm">
                        		<i class='fa fa-edit'></i> <?php if(cv('shop.nav.edit')) { ?>修改<?php  } else { ?>查看<?php  } ?>
                        	</a>
                        <?php  } ?>
                        <?php if(cv('shop.nav.delete')) { ?>
                        	<a data-toggle='ajaxRemove' href="<?php  echo webUrl('shop/nav/delete', array('id' => $row['id'], 'group_id' => $group_id))?>"class="btn btn-default btn-sm" data-confirm='确认要删除此导航吗?'><i class="fa fa-trash"></i> 删除</a>
                        <?php  } ?>
                    </td>
                </tr>
                <?php  } } ?> 
