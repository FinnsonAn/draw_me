...
/* 数据库数组转换为字符串 */
function dataArrayToStr($array, $key = 'id', $code = ',')
{
	...
	if (is_array($new_arr))
	{
		$str = implode($code, $new_arr);
	}
	return $str;
}

/* 价格格式化 */
function priceFormat($data, $n = 2)
{
	if (is_array($data))
	{
		foreach ($data as &$v)
...
function priceFormat1($price, $n = 2)
{
	$price = round($price, $n);
	return $price;
}

function emptyShowZero($val)
{
	if (!$val || empty($val))
	{
		$val = 0;
