...
class Sale_EweiShopV2ComModel extends ComModel 
{
	...
	/* 积分转换金额 */
	public function integralToMoney($integral)
	{
		if (!$integral) return 0;
		$saleset = m('common')->getPluginset('sale');
		$pmoney  = round(floatval($saleset['money']), 2);
		$money = round($integral * $pmoney, 2);
		return $money;
	}
	
	/* 金额转换积分 */
	public function moneyToIntegral($money)
	{
		$saleset = m('common')->getPluginset('sale');
		$pmoney  = round(floatval($saleset['money']), 2);
		if (!$money || !$pmoney) return 0;
		$integral = $money / $pmoney;
		return $integral;
	}

	/* 满额减 */
	public function fullReduction($totalprice)
	{
		$arr = array('discount' => 0, 'info' => '');
		$saleset = m('common')->getPluginset('sale');
		if ($saleset['enoughs'])	
		{
			foreach ($saleset['enoughs'] as $e) 
			{
				if ($totalprice >= floatval($e['enough']) && floatval($e['give']) > 0) 
				{
					$arr['discount'] = floatval($e['give']);
					$arr['info'] = '订单满'. $e['enough'] .'元减' . $e['give'] . '元';
					break;
				}
			}
		}
