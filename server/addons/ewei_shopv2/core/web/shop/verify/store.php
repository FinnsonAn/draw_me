...
class Store_EweiShopV2Page extends ComWebPage 
{
	...
	protected function post() 
	{
		...
		if ($_W['ispost']) 
		{
			$data = array('uniacid' => $_W['uniacid'], 'storename' => trim($_GPC['storename']), 'address' => trim($_GPC['address']), 'tel' => trim($_GPC['tel']), 'lng' => $_GPC['map']['lng'], 'lat' => $_GPC['map']['lat'], 'type' => intval($_GPC['type']), 'realname' => trim($_GPC['realname']), 'mobile' => trim($_GPC['mobile']), 'wechat' => trim($_GPC['wechat']), 'qq' => trim($_GPC['qq']), 'fetchtime' => trim($_GPC['fetchtime']), 'saletime' => trim($_GPC['saletime']), 'desc' => trim($_GPC['desc']), 'status' => intval($_GPC['status']));
			$data['web'] = trim($_GPC['web']);
			...
			if (is_array($_GPC['logos'])) 
			{
				...
				$data['logo'] = save_media($thumb_url[0]);
				unset($thumb_url[0]);
				$data['logo_url'] = serialize(m('common')->array_images($thumb_url));
			}
			if (!(empty($id))) 
			{
		...
		if (!(empty($item['logo']))) 
		{
			$piclist = array_merge(array($item['logo']), iunserializer($item['logo_url']));
		}
