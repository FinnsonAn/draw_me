...
<div class="page-heading"> 
    <span class='pull-right'>
        <?php if(cv('shop.notice.add')) { ?>
        	<a class='btn btn-primary btn-sm' href="<?php  echo webUrl('shop/notice/add', array('cid' => $cid))?>"><i class='fa fa-plus'></i> 添加文章</a>
        <?php  } ?>
    </span>
    <h2>文章管理</h2> 
</div>
        ...
        <div class="col-sm-4">
            ...
            <div class="input-group-btn">
                ...
                <?php if(cv('shop.notice.edit')) { ?>
	                <button class="btn btn-default btn-sm" type="button" data-toggle='batch' data-href="<?php  echo webUrl('shop/notice/status',array('status'=>1, 'cid' => $cid))?>"><i class='fa fa-circle'></i> 显示</button>
	                <button class="btn btn-default btn-sm" type="button" data-toggle='batch'  data-href="<?php  echo webUrl('shop/notice/status',array('status'=>0, 'cid' => $cid))?>"><i class='fa fa-circle-o'></i> 隐藏</button>
                <?php  } ?>
                <?php if(cv('shop.notice.delete')) { ?>	
                	<button class="btn btn-default btn-sm" type="button" data-toggle='batch-remove' data-confirm="确认要删除?" data-href="<?php  echo webUrl('shop/notice',array('op'=>'delete', 'cid' => $cid))?>"><i class='fa fa-trash'></i> 删除</button>
                <?php  } ?>
        ...
        <div class="col-sm-6 pull-right">
            ...
            <select name='position_id' class='form-control input-sm select-sm' style="width:150px;">
              <option value="">
                ...
                <option value="<?php  echo $v['id'];?>"<?php  if($item['cate_id']==$v['id'] || $cid==$v['id'] ) { ?> selected=""<?php  } ?>><?php  echo $v['title'];?></option>
                <?php  } } ?>
            </select>
                ...
                <td>
                    ...
                    <td style="text-align:left;">
                        <?php if(cv('shop.notice.view|shop.notice.edit')) { ?>
                        	<a data-toggle="ajaxModal" href="<?php  echo webUrl('shop/notice/qr',array('id' => $row['id']))?>" class="btn btn-default btn-sm" >
                        		<i class='fa fa-qrcode'></i> 二维码
                        	</a>
                        	<a href="<?php  echo webUrl('shop/notice/edit',array('id' => $row['id'], 'cid' => $cid))?>" class="btn btn-default btn-sm" >
                        		<i class='fa fa-edit'></i> <?php if(cv('shop.notice.edit')) { ?>修改<?php  } else { ?>查看<?php  } ?>
                        	</a>
                        <?php  } ?>
                        <?php if(cv('shop.notice.delete')) { ?>
                        	<a data-toggle='ajaxRemove' href="<?php  echo webUrl('shop/notice/delete', array('id' => $row['id'], 'cid' => $cid))?>"class="btn btn-default btn-sm" data-confirm='确认要删除此文章吗?'><i class="fa fa-trash"></i> 删除</a>
                        <?php  } ?>
                    </td>
                </tr>
                <?php  } } ?>
