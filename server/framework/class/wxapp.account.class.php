...
class WxappAccount extends WeAccount {
	...
	public function pkcs7Encode($encrypt_data, $iv) {
		...
		$result = $this->pkcs7Decode($encrypt_data, $_SESSION['session_key'], $iv);
		if (is_error($result)) {
			return error(1, '解密失败');
		}
	...
	public function pkcs7Decode($encrypt_data, $key, $iv)
	{
	...
	public function getAccessToken() {
		...
		$record['expire'] = time() + $response['expires_in'] - 1000;
	...
	public function getCodeWithPath($path, $width=430) {
