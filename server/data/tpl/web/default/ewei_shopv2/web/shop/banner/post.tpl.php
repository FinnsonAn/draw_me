...
<div class="page-heading"> 
    ...
    <form <?php if( ce('shop.banner' ,$item) ) { ?>action="" method="post"<?php  } ?> class="form-horizontal form-validate" enctype="multipart/form-data">
        <input type="hidden" name="id" value="<?php  echo $item['id'];?>" />
                ...
                <div class="form-group">
                    <label class="col-sm-2 control-label must">所属位置</label>
                    <div class="col-sm-9 col-xs-12">
                        <select class="form-control" name="position_id" id="position_id">
                            <option value="">请选择</option>
                            <?php  if(is_array($positions)) { foreach($positions as $v) { ?>
                            <option value="<?php  echo $v['pid'];?>" <?php  if($item['position_id']==$v['pid'] || $position_id==$v['pid']) { ?> selected=""<?php  } ?>><?php  echo $v['position_name'];?><?php  if($v['size']) { ?>(<?php  echo $v['size'];?>)<?php  } ?></option>
                            <?php  } } ?>
                        </select>
                    </div>
                </div>
