...
<div class="page-heading"> 
    <span class='pull-right'>
        <?php if(cv('shop.notice.add')) { ?>
        	<a class="btn btn-primary btn-sm" href="<?php  echo webUrl('shop/notice/add')?>">添加新文章</a>
        <?php  } ?>
        <a class="btn btn-default  btn-sm" href="<?php  echo webUrl('shop/notice')?>">返回列表</a>
    </span>
    <h2><?php  if(!empty($notice['id'])) { ?>编辑<?php  } else { ?>添加<?php  } ?>文章 <small><?php  if(!empty($notice['id'])) { ?>修改【<?php  echo $notice['title'];?>】<?php  } ?></small></h2> 
</div> 
...
<form <?php if( ce('shop.notice' ,$notice) ) { ?>action="" method="post"<?php  } ?> class="form-horizontal form-validate" enctype="multipart/form-data" onsubmit='return formcheck()'>
    ...
    <div class="form-group">
        <label class="col-sm-2 control-label must">所属分类</label>
        <div class="col-sm-9 col-xs-12">
            <select class="form-control" name="cate_id" id="cate_id">
                <option value="">请选择</option>
                <?php  if(is_array($cates)) { foreach($cates as $v) { ?>
                <option value="<?php  echo $v['id'];?>"<?php  if($notice['cate_id']==$v['id'] || $cid==$v['id'] ) { ?> selected=""<?php  } ?>><?php  echo $v['title'];?></option>
                <?php  } } ?>
            </select>
        </div>
    </div>
