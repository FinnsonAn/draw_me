...
class Op_EweiShopV2Page extends WebPage
{
	...
	public function setauthor()
	{
		...
		if ($item['status'] != 1 && $item['status'] != 2) {
			show_json(0, '该状态不能分配画师！');
		}
		if ($item['author'] > 0)
		{
			$author = p('case')->getAuthor($item['author']);
		}
		if ($_W['ispost']) {
			$authorid = intval($_GPC['authorid']);
			pdo_update('ewei_shop_order', array('author' => $authorid), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			show_json(1);
		}
		include $this->template();
	}
