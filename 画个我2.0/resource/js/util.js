...
util.url = function (action, querystring) {
	var app = getApp();
  var url = app.siteInfo.siteroot + '?i='+ app.siteInfo.uniacid + '&uniacid=' + app.siteInfo.uniacid + '&t=' + app.siteInfo.multiid + '&v=' + app.siteInfo.version + '&m=' + app.siteInfo.module + '&from=wxapp&';
            ...
            if (!session.data.errno) {
              ...
              /*
                    ...
                    dataType: 'json',
                      ...
                      var data = user_res.data;
                      if (!data.errno) {
                        userInfo.memberInfo = data.data.member;
												wx.setStorageSync('userInfo', userInfo);
											}
											typeof cb == "function" && cb(userInfo);
										}
...
*/
util.message = function(title, redirect, msgtype) {
	if (!title) {
		return true;
	}
