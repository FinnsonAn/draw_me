...
if ($do == 'openid') {
	...
	if (!empty($oauth) && !is_error($oauth)) {
		...
		if (empty($fans)) {
			...
			if (!empty($email_exists_member)) {
				...
				$default_groupid = pdo_fetchcolumn('SELECT groupid FROM ' .tablename('mc_groups') . ' WHERE uniacid = :uniacid AND isdefault = 1', array(':uniacid' => $_GPC['uniacid']));
				$data = array(
					'uniacid' => $_GPC['uniacid'],
					'email' => $email,
					'salt' => random(8),
	...
	$iv = $_GPC['iv'];
	logging_run('encrypt_data' . $_GPC['encrypt_data'] . ' iv' . $iv);
	...
	logging_run('sign' . $sign . ' signature' . $_GPC['signature']);
