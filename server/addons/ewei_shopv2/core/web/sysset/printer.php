...
class Printer_EweiShopV2Page extends WebPage 
{
	...
	public function set() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$data['printer_pay'] = intval($_GPC['printer_pay']);
			$data['printer_recharge'] = intval($_GPC['printer_recharge']);
			$data['printer_offline'] = intval($_GPC['printer_offline']);
			$data['mem_fontsize'] = intval($_GPC['mem_fontsize']);
			$data['mem_width'] = intval($_GPC['mem_width']);
	...
	{
		com_run('printer::sendOrderMessage', 267);
	}
}
?>