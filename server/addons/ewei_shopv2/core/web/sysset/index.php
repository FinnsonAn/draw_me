...
class Index_EweiShopV2Page extends WebPage 
{
	public function main() 
	{
		...
		else if (cv('sysset.wxapp')) 
		{
			header('location: ' . webUrl('sysset/wxapp'));
		}
	...
	public function shop() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$data['uc_bg'] = save_media($data['uc_bg']);
			$data['nothumb'] = save_media($data['nothumb']);
			$data['contact_bg'] = save_media($data['contact_bg']);
			$data['recharge_bg'] = save_media($data['recharge_bg']);
			...
			$data['webtype'] = intval($data['webtype']);
			m('common')->updateSysset(array('shop' => $data));
			plog('sysset.shop.edit', '修改系统设置-商城设置');
			show_json(1, array('url' => webUrl('sysset/shop')));
		}
	...
	public function wxapp() 
	{
		global $_W;
		global $_GPC;
		$data = m('common')->getSysset('wxapp');
		if ($_W['ispost']) 
		{
			ca('sysset.wxapp.edit');
			$data = ((is_array($_GPC['data']) ? $_GPC['data'] : array()));
			$data['name'] = trim($data['name']);
			$data['logo'] = save_media($data['logo']);
			m('common')->updateSysset(array('wxapp' => $data));
			plog('sysset.wxapp.edit', '修改系统设置-小程序设置');
			show_json(1, array('url' => webUrl('sysset/wxapp')));
		}
		include $this->template();
	}
	public function wxtmpl() 
	{
		global $_W;
		global $_GPC;
		$data = m('common')->getSysset('wxtmpl');
		if ($_W['ispost']) 
		{
			ca('sysset.wxtmpl.edit');
			$data = ((is_array($_GPC['data']) ? $_GPC['data'] : array()));
			m('common')->updateSysset(array('wxtmpl' => $data));
			show_json(1, array('url' => webUrl('sysset/wxtmpl')));
		}
		include $this->template();
	}
	public function follow() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			show_json(1, array('url' => webUrl('sysset/share')));
		}
	...
	public function trade() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$data['paybtn'] = intval($data['paybtn']);
			$data['beta_paybtn'] = intval($data['beta_paybtn']);
			...
			$data['offline_pay_desc'] = trim($data['offline_pay_desc']);
	...
	public function payset() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$data['offline_pay'] = intval($inputdata['offline_pay']);
	...
	public function member() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$data['bind_old_member'] = intval($data['bind_old_member']);
