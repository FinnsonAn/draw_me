...
class EweiShopWechatPay
{
	...
	public function order()
	{
		...
		$sql = 'SELECT * FROM ' . tablename('core_paylog') . ' WHERE module=\'ewei_shopv2\' AND tid=\''.$tid.'\'  limit 1';
		$log = pdo_fetch($sql);
		if (!(empty($log)) && (($log['status'] == 0) || $ispeerpay) && (($log['fee'] == floatval($this->total_fee)) || $ispeerpay)) {
	...
	public function recharge()
	{
		...
		$log = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_member_log') . ' WHERE `uniacid`=:uniacid and `id`=:logno limit 1', array(':uniacid' => $_W['uniacid'], ':logno' => $logno));
