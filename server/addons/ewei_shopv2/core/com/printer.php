...
class Printer_EweiShopV2ComModel extends ComModel
{
	...
	public function style_list_code($str, $code = 0, $type = 0)
	{
		if (($type == '0') || ($type == '1')) {
			...
			case self::CONTENT_CENTER:
				$res = '<FH><center>' . trim($str) . '</center></FH>';
				break;

			case self::CONTENT_CENTER_BOLD:
				$res = '<FH><center><FB>' . trim($str) . '</FB></center>' . "\n</FH>";
				break;
	...
	public function setSpacing($str, $length = 32)
	{
		...
		if ($length < $num) {
			...
			return mb_substr($str_old, 0, floor($length / 2), 'utf-8') . str_repeat(' ', $length % 2+1);
		}
	...
	public function sendOrderMessage($orderid = 0, $verify = array())
	{
		...
		$buyerinfo_name = $order['realname'];
		$buyerinfo_mobile = $order['mobile'];
		$addressinfo = $order['province'] . ' ' . $order['city'] . ' ' . $order['erea'] . ' ' . $order['address'];
		...
		if ($shopset['webtype'] == 1)  // 餐饮行业
		{
			...
			if ($order['reservetime'] > 0)
			{
				$order['reservetime_format'] = date('Y-m-d H:i', $order['reservetime']);
				$other_param .= '[用餐时间] ' . $order['reservetime_format'] . '\n';
			}
			if ($order['customer_num'])
			{
				$other_param .= '[用餐人数] ' . $order['customer_num'] . '人\n';
			}
			if ($order['usetype'])
			{
				$other_param .= '[用餐方式] ' . $order['usetype']['val'] . '\n';
			}
			$order['payment'] = m('shop')->getPaymentByIndex($order['paytype']);
			$other_param .= '[支付方式] ' . $order['payment']['name'] . '\n';
			$order['remark'] = $other_param . '[留言] ' . $order['remark'];
